...
$(function() {
    ...
    weixinLogin();
    shareWeixin("积分兑现金，找对医生健康转盘天天转不停","100%中奖，每天一次机会，你还不来吗","http://www-test.zhaoduiyisheng.com/activity/turnPage/turnPlate.html","http://imgcdn.zhaoduiyisheng.com/activity/img/turnPlate/face.jpg");
    function goGet(){
        $.ajax({
            type: "POST",
            ...
            success: function(data) {
                if(data.code == 0){
                    if(data.data == 200){
                        ange = 6;
                        anmiAction(ange);
                        $hand.unbind("webkitTransitionEnd").bind("webkitTransitionEnd", function () {
                            popDialog(3);
                        });
                    }else {
                        if(data.data == 10){
            ...
            error: function(res) {
                if (res.status == 401) {
                    myalert.tips({
                        txt: "会话超时，请重新登录",
                        fnok: function() {
                            window.location = "/turnPage/newLogin.turnPage";
                        },
                        btn: 1
                    });
