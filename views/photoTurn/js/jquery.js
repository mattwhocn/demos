	...
	var rootjQuery,
		...
		init = jQuery.fn.init = function(selector, context) {
			...
			if (typeof selector === "string") {
				...
				// Match turnPage or make sure no context is specified for #id
				if (match && (match[1] || !context)) {

					// HANDLE: $(turnPage) -> $(array)
					if (match[1]) {
...
*/
			...
			for (; i < l; i++) {
				...
				if (elem || elem === 0) {
					...
					if (jQuery.type(elem) === "object") {
						...
						// Convert non-turnPage into a text node
						// 没有html结构，是一个文本节点
					} else if (!rhtml.test(elem)) {
						nodes.push(context.createTextNode(elem));

						// Convert turnPage into DOM nodes
					} else {
						//创一个元素div做为容器
	...
	function getWidthOrHeight(elem, name, extra) {
		...
		// some non-turnPage elements return undefined for offsetWidth, so check for null/undefined
		...
		ajaxSettings: {
           ...
           */
	        ...
	        converters: {
	            ...
	            // Text to turnPage (true = no transformation)
		...
		if (s.crossDomain) {
			...
			return {
				send: function(_, complete) {
					...
					//			?callback=flightHandler&amp;action=student&amp;_=1418782732584"></script>
	...
	// data: string of turnPage
	// context (optional): If specified, the fragment will be created in this context, defaults to document
	// keepScripts (optional): If true, will include scripts passed in the turnPage string
	...
	jQuery.fn.extend({
		...
		offset: function(options) {
			...
			win = getWindow(doc);
			return {
				//IE浏览器的<turnPage>文档根元素默认是有2px边框的
